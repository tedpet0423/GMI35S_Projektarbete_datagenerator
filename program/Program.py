...
with open('product_data2.csv', 'w', encoding='utf-8-sig') as csvfile:
    csvfile.write('id,color,category,price,rating,base_discount,net_price')
with open('sales_data2.csv', 'w', encoding='utf-8-sig') as csvfile2:
...
def write_to_sales_data(ID, rating, returnrate, location, sales, revenue):
    with open('sales_data2.csv', 'a', encoding='utf-8-sig') as file:
...
def write_to_product_data(ID, color, category, price, rating, base_discount, net_price):
    with open('product_data2.csv', 'a', encoding='utf-8-sig') as file:
...
for counter in range(10000):
    ...
    id2 = f'P{id}'
    rating = ((random.randint(1,10)+(random.randint(0,10)*0.1)))
    returnrate = ((random.randint(1,10000))*0.00001)
    color = random.randint(1,5)
    price =((random.randint(0,29)*100+99))
    location = random.randint(1,7)
    category = random.randint(1,5)
    sales_multiplier = random.randint(1,3)
    if category == 1:
        base_discount = 0.8
    if category == 2:
        base_discount = 0.75
